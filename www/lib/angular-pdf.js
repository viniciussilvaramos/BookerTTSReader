  ...
  angular.module('pdf', []).directive('ngPdf', [ '$window', function($window) {
    ...
    return {
      ...
      link: function(scope, element, attrs) {
        ...
        scope.getTextFromPage = function (num) {
          return pdfDoc.getPage(num).then(function(page) {
              return page.getTextContent();
          })
        };
          
        scope.apenasTexto = true;
        ...
        scope.renderPage = function(num) {
          ...
          pdfDoc.getPage(num).then(function(page) {
              ...
              if(scope.apenasTexto){
                  page.getTextContent({normalizeWhitespace:true}).then(function (texto) {

                    var linhas = [];
                    console.log(texto);
                    var linha = '';
                    for(var i = 0; i < texto.items.length; i++){

                      linha += texto.items[i].str;

                      var nextI = i+1;
                      if(nextI < texto.items.length){
                        if(texto.items[i].height !== texto.items[nextI].height){
                          linhas.push({texto:linha,tamanho:texto.items[i].height});
                          linha = '';
                        }
                      }else{
                        linhas.push({texto:linha,tamanho:texto.items[i].height});
